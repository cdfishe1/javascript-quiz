...
const generateQuestions = () => {
  ...
  const i = Math.floor(Math.random() * questionsArray.length)
...
variables.saveButton.addEventListener('click', function (event) {
  ...
  const oldScores = JSON.parse(localStorage.getItem('storeScores')) || []
...
const renderScores = () => {
  const lastScore = JSON.parse(localStorage.getItem('storeScores'))
